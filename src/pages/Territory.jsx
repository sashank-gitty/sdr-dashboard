...
import { PageHeader, Card, Pill, SectionTitle, EmptyState, SectionNote } from "../components/ui.jsx"
...
function Stat({ label, value, sub, tone }) {
  ...
  return (
    <div className="border-r border-slate-200 px-4 py-3 last:border-r-0 dark:border-zinc-800">
      ...
      {sub && <p className="mt-0.5 text-[11.5px] text-slate-400 dark:text-zinc-500">{sub}</p>}
    </div>
  )
}
...
function BarRow({ label, sub, value, max, display, tone = "indigo" }) {
  const width = max > 0 ? Math.max((Math.abs(value) / max) * 100, 2) : 2
  const fill = tone === "loss" ? "bg-rose-500" : "bg-indigo-500"
  return (
    <div className="flex items-center gap-3">
      <div className="w-44 flex-shrink-0">
        <p className="truncate text-[13px] font-medium text-slate-800 dark:text-zinc-200">{label}</p>
        {sub && <p className="truncate text-[11px] text-slate-400 dark:text-zinc-500">{sub}</p>}
      </div>
      ...
      <span className="w-20 flex-shrink-0 text-right font-mono text-[12.5px] font-semibold tabular-nums text-slate-600 dark:text-zinc-300">
        {display}
      </span>
    </div>
  )
...
function AccountTable({ columns, rows, empty }) {
  if (!rows.length) return <p className="px-1 py-3 text-[13px] text-slate-500 dark:text-zinc-400">{empty}</p>
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[560px] text-[13px]">
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/70">
          {rows.map((r, i) => (
            <tr key={r.account_key ?? i}>
              {columns.map((c) => (
                <td
                  key={c.key}
                  className={`py-2 pr-3 align-baseline ${
                    c.num ? "text-right font-mono tabular-nums text-slate-600 dark:text-zinc-300" : ""
                  }`}
                >
...
function Territory() {
    ...
    return (
      <>
        ...
        <div className="space-y-3">
          <div className="h-24 animate-pulse rounded-xl bg-slate-100 dark:bg-zinc-800/60" />
    ...
    return (
      <>
        ...
        <Card className="p-5">
          ...
          <p className="mt-1 text-[12.5px] text-slate-500 dark:text-zinc-400">{state.message}</p>
        </Card>
      </>
    )
  }
  ...
  return (
    <>
      ...
      <Card className="mb-6 grid grid-cols-2 overflow-hidden lg:grid-cols-5">
        <Stat label="Accounts" value={(t.accounts ?? 0).toLocaleString()} sub="across all books" />
        <Stat
          label="Customers"
          value={(t.customers ?? 0).toLocaleString()}
          sub={`${pct(t.customers, t.accounts)} penetration`}
        />
        <Stat label="Total ARR" value={fmtMoney(t.arr)} sub={`avg ${fmtMoney(t.customers ? t.arr / t.customers : 0)}`} tone="accent" />
        <Stat label="Open pipeline" value={fmtMoney(t.open_pipeline)} sub="software + research" />
        <Stat
      ...
      {gap > 1 && (
        <Card className="mb-6 border-indigo-500/30 bg-indigo-500/5 p-5">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">
            The cross-sell case
          </p>
          <p className="mt-1.5 text-[15px] leading-relaxed text-slate-700 dark:text-zinc-200">
            Customers on more than one product line average{" "}
            <span className="font-mono font-semibold text-slate-900 dark:text-zinc-50">{fmtMoney(multiAvg)}</span>.
            Single-line customers average{" "}
            <span className="font-mono font-semibold text-slate-900 dark:text-zinc-50">{fmtMoney(singleAvg)}</span> &mdash;
            a <span className="font-semibold text-indigo-600 dark:text-indigo-400">{gap.toFixed(1)}&times; gap</span>.
            There are {mix.single_count} single-line customers holding {fmtMoney(mix.single_arr)}.
          </p>
        </Card>
      )}
          ...
          <table className="w-full min-w-[560px] text-[13px]">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/70">
              {byVertical.map((v) => {
                ...
                return (
                  <tr key={v.vertical}>
                    <td className="py-2 pr-3 font-medium text-slate-800 dark:text-zinc-200">{v.vertical}</td>
                    <td className="py-2 pr-3 text-right font-mono tabular-nums text-slate-600 dark:text-zinc-300">{v.accounts}</td>
                    <td className="py-2 pr-3 text-right font-mono tabular-nums text-slate-600 dark:text-zinc-300">{v.customers}</td>
                    <td className="py-2 pr-3 text-right font-mono tabular-nums text-slate-600 dark:text-zinc-300">
                    ...
                    <td
                      className={`py-2 text-right font-mono font-semibold tabular-nums ${
                        losing ? "text-rose-600 dark:text-rose-400" : "text-slate-700 dark:text-zinc-200"
                      }`}
                    >
      ...
      <Card className="mb-6 p-5">
        <AccountTable
          ...
          rows={crossSell}
          columns={[
            { key: "account_name", label: "Account", render: (r) => <span className="font-medium text-slate-800 dark:text-zinc-200">{r.account_name}</span> },
            { key: "ae", label: "AE" },
            ...
            { key: "arr", label: "Current ARR", num: true, render: (r) => fmtMoney(r.arr) },
            {
              ...
              render: (r) => {
                ...
                return <Pill tone="indigo">{missing.join(" / ")}</Pill>
              },
            },
          ]}
        />
